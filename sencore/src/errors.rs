...
fn show_source_context(output: &mut String, source: &str, span: Span<usize>) {
    let start_loc = byte_offset_to_location(source, span.start);
    let end_loc = byte_offset_to_location(source, span.end.saturating_sub(1).max(span.start));
...
pub fn format_error_stack(source: &str, error: &InterpretError) -> String {
    let mut output = String::new();

    writeln!(output, "Error Trace:").unwrap();

    // Reverse iteration: last pushed (outermost) comes first
    for span in error.spans.iter().rev() {
        let loc = byte_offset_to_location(source, span.start);
        writeln!(output, "  at {}:{}", loc.line, loc.column).unwrap();
        show_source_context(&mut output, source, *span);
        writeln!(output).unwrap();
    }
